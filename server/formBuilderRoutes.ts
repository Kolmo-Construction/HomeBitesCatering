...
router.post("/forms/:formId/rules", async (req, res) => {
  try {
    ...
    const createRuleSchema = z.object({
      triggerFormPageQuestionId: z.number(),
      conditionType: z.enum(formSchema.conditionalLogicConditionTypeEnum.enumValues),
      conditionValue: z.string().nullable().optional(),
      actionType: z.enum(formSchema.conditionalLogicActionTypeEnum.enumValues),
      targets: z.array(z.object({
        targetType: z.enum(formSchema.formRuleTargetTypeEnum.enumValues),
        targetId: z.number()
      })),
      ruleDescription: z.string().nullable().optional(),
      executionOrder: z.number().nullable().optional()
    });

    // Validate the request body
    const parsedData = createRuleSchema.safeParse(req.body);
    if (!parsedData.success) {
      console.error("Rule validation error:", parsedData.error.format());
      return res.status(400).json({ 
    ...
    const { targets, ...ruleData } = parsedData.data;

    // Use a transaction for atomicity
    const result = await db.transaction(async (tx) => {
      // 1. Create the rule
      const [newRule] = await tx.insert(formSchema.formRules)
        .values({
          formId,
          ...ruleData
        })
      ...
      // 2. Create the rule targets
      const ruleTargets = await Promise.all(
        targets.map(async (target) => {
          const [newTarget] = await tx.insert(formSchema.formRuleTargets)
            .values({
              ruleId: newRule.id,
              targetType: target.targetType,
              targetId: target.targetId
...
// 2. Get All Rules for a Specific Form
router.get("/forms/:formId/rules", async (req, res) => {
  try {
    const formId = parseInt(req.params.formId);
    if (isNaN(formId)) {
...
router.get("/questions/:questionId/rules", async (req, res) => {
  try {
    ...
    // Fetch all rules where this question is the trigger
    const rules = await db.query.formRules.findMany({
...
router.put("/rules/:ruleId", async (req, res) => {
  try {
    ...
    const updateRuleSchema = z.object({
      conditionType: z.enum(formSchema.conditionalLogicConditionTypeEnum.enumValues).optional(),
      conditionValue: z.string().nullable().optional(),
      actionType: z.enum(formSchema.conditionalLogicActionTypeEnum.enumValues).optional(),
      targets: z.array(z.object({
        targetType: z.enum(formSchema.formRuleTargetTypeEnum.enumValues),
        targetId: z.number()
      })).optional(),
    ...
    // Validate the request body
    const parsedData = updateRuleSchema.safeParse(req.body);
    if (!parsedData.success) {
      console.error("Rule update validation error:", parsedData.error.format());
      return res.status(400).json({ 
    ...
    const result = await db.transaction(async (tx) => {
      // 1. Update the rule
      const [updatedRule] = await tx.update(formSchema.formRules)
        .set({
          ...ruleData,
          updatedAt: new Date()
        })
      ...
      // 2. If targets are provided, update them
      if (targets && targets.length > 0) {
        // First delete existing targets
        await tx.delete(formSchema.formRuleTargets)
          .where(eq(formSchema.formRuleTargets.ruleId, ruleId));
      ...
      // If no targets were provided, fetch existing ones
      const existingTargets = await tx.query.formRuleTargets.findMany({
        where: eq(formSchema.formRuleTargets.ruleId, ruleId)
      });
...
router.delete("/rules/:ruleId", async (req, res) => {
  try {
    ...
    // Use a transaction for atomicity
    await db.transaction(async (tx) => {
      // First delete all targets for this rule
      await tx.delete(formSchema.formRuleTargets)
        .where(eq(formSchema.formRuleTargets.ruleId, ruleId));
      
      // Then delete the rule itself
      await tx.delete(formSchema.formRules)
        .where(eq(formSchema.formRules.id, ruleId));
    });
