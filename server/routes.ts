...
Return ONLY the JSON object with endpoint, method, and json fields. The json field should be a VALID JSON string that can be parsed.`
  ...
  // Condition visualizer endpoints
  
  app.get('/api/visualizer/questionnaire/:definitionId', isAdmin, async (req: Request, res: Response) => {
    try {
      const definitionId = parseInt(req.params.definitionId);
      if (isNaN(definitionId)) {
        return res.status(400).json({ success: false, message: 'Invalid definition ID' });
      }
      
      // Fetch data directly from storage
      const definition = await storage.getQuestionnaireDefinition(definitionId);
      if (!definition) {
        return res.status(404).json({ success: false, message: 'Questionnaire definition not found' });
      }
      
      const pages = await storage.getQuestionnairePagesByDefinition(definitionId);
      
      // Build an array of all questions from all pages
      const questions = [];
      for (const page of pages) {
      ...
      const conditionalLogic = await storage.getConditionalLogicRulesByDefinition(definitionId);
      ...
      return res.status(500).json({ success: false, message: 'Error fetching visualizer data' });
    }
  });
  
  app.get('/api/visualizer/questionnaire/:definitionId/analyze', isAdmin, async (req: Request, res: Response) => {
    try {
      const definitionId = parseInt(req.params.definitionId);
      if (isNaN(definitionId)) {
        return res.status(400).json({ success: false, message: 'Invalid definition ID' });
      ...
      const visualizerData = await getVisualizerData(definitionId);
      const analysis = analyzeConditionalLogic(visualizerData);
      ...
      console.error('Error analyzing conditional logic:', error);
      return res.status(500).json({ success: false, message: 'Error analyzing conditional logic' });
    }
  });
